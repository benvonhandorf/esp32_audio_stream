...
esp_err_t audio_output_init(audio_output_context_t* ctx) {
    ...
    // Create output queue
    ctx->output_queue =
        xQueueCreate(AUDIO_OUTPUT_QUEUE_SIZE, sizeof(audio_output_buffer_t*));
    ...
    chan_cfg.dma_frame_num = 800;

    esp_err_t ret = i2s_new_channel(&chan_cfg, &ctx->i2s_tx_chan, NULL);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to create I2S TX channel: %s",
                 esp_err_to_name(ret));
    ...
    i2s_std_config_t std_cfg = {
        ...
        .slot_cfg = I2S_STD_PHILIPS_SLOT_DEFAULT_CONFIG(I2S_DATA_BIT_WIDTH_8BIT,
                                                        I2S_SLOT_MODE_MONO),
        .gpio_cfg =
            {
    ...
    ESP_LOGI(TAG, "GPIO - BCLK: %d, LRCLK: %d, SDATA: %d", I2S_TX_BCLK_GPIO,
             I2S_TX_LRCLK_GPIO, I2S_TX_SDATA_GPIO);
...
esp_err_t audio_output_start(audio_output_context_t* ctx) {
    ...
    i2s_std_gpio_config_t gpio_cfg = {.mclk = I2S_GPIO_UNUSED,
                                      .bclk = I2S_TX_BCLK_GPIO,
                                      .ws = I2S_TX_LRCLK_GPIO,
                                      .dout = I2S_TX_SDATA_GPIO,
                                      .din = I2S_GPIO_UNUSED,
...
esp_err_t audio_output_stop(audio_output_context_t* ctx) {
    ...
    ctx->is_playing = false;
    ESP_LOGI(TAG, "Audio playback stopped. Total bytes played: %lu",
             ctx->bytes_played);
...
// ============================================================================
esp_err_t audio_output_queue_data(audio_output_context_t* ctx,
    ...
    if (size > AUDIO_OUTPUT_BUFFER_SIZE) {
        ESP_LOGE(TAG, "Data size %d exceeds buffer size %d", size,
...
esp_err_t audio_output_chirp_up(audio_output_context_t* ctx) {
    ...
    esp_err_t res =
        audio_output_queue_data(ctx, chirp_up_raw, chirp_up_raw_len);
...
esp_err_t audio_output_chirp_down(audio_output_context_t* ctx) {
    ...
    esp_err_t res =
        audio_output_queue_data(ctx, chirp_down_raw, chirp_down_raw_len);
    ...
    while (1) {
        ...
        if (xQueueReceive(ctx->output_queue, &buf, portMAX_DELAY) == pdTRUE) {
            if (buf != NULL) {
                if (ctx->is_playing) {
                    // Write data to I2S
                    ret = i2s_channel_write(ctx->i2s_tx_chan, buf->data,
                    ...
                    if (xQueuePeek(ctx->output_queue, &buf, 0) ==
                        errQUEUE_EMPTY) {
